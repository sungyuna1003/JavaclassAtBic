...
public class ChatServer2 {
	...
	class CThread2 extends Thread {
		...
		public void run() {
			try {
				out.println("Enter the ID you want to use.");
				while(true) {
		...
		//Client로 부터 전송된 프로토콜 및 data 분석 메소드
		public void routine(String line) {
			...
			if(cmd.equals(ChatProtocol2.ID)) {
				...
				//MESSAGE:bbb;오늘은 짜장면 OK?
				idx = data.indexOf(';');
				cmd = data.substring(0, idx);//bbb
				data = data.substring(idx+1);//오늘은 짜장면 OK?
				CThread2 ct = findClient(cmd);
				if(ct!=null) {
					//bbb에게 쪽지를 보낸다.
		...
		public CThread2 findClient(String id) {
			...
			for (int i = 0; i < vc.size(); i++) {
				ct = vc.get(i);
				//찾고자하는  id가 일치하면 뒤에 있는 것은 검색 불필요.
				if(ct.id.equals(id))
					break;
			}
			return ct;
		...
		//접속된 모든 id를 리턴(구분자는 ;  ex)aaa;bbb;홍길동; )
